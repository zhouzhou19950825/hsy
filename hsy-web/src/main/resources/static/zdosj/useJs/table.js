	...
	for (var i = 0; i < menuNodes.length; i++) {
		...
		if (useMenuNode.myId === menuNode.myId) {
			...
			if (menuNodes[i].isLeaf === 1) {
				...
				deleteByMenuNodeId(menuNodes[i].myId, "areaTableData",
						"AreaTableData");
				deleteByMenuNodeId(menuNodes[i].myId, "tableHeadNode",
						"TableHeadNode");
			}
...
// 根据菜单节点
function deleteByMenuNodeId(deletedMenuNodeId, deletedNodeName,
		deletedNodeTable) {
	$.ajax({
		type : "GET",
		url : "/" + deletedNodeName + "/delete" + deletedNodeTable
				+ "ByMenuNodeId",
		async : false, // 取消异步请求
		data : {
...
function zTreeOnClick(event, treeId, treeNode) {
	...
	if (!treeNode.isParent) {
		...
		addHtml += "<button id='updateAreaTableDatasAAA' class='btn btn-small' onclick=updateAreaTableDatas()>保存</button>";
...
function addAreaTableData() {
	...
	for (var i = 0; i < countries.length; i++) {
		addTableHtml += "<tr><td id='" + countries[i].id + "'>"
				+ countries[i].countryName + "</td>";
		for (var j = 0; j < sortLeafNodes.length; j++) {
			if (sortLeafNodes[j].id != -1) {
				addTableHtml += "<td id='" + countries[i].id + "_"
						+ sortLeafNodes[j].id
	...
	for (var i = 0; i < countries.length; i++) {
		for (var j = 0; j < sortLeafNodes.length; j++) {
			if (sortLeafNodes[j].id != -1) {
				getAreaTableDataByCountryIdAndTableHeadNodeId(countries[i].id,
						sortLeafNodes[j].id);
			}
		}
	}
...
function updateAreaTableDatas() {
	for (var i = 0; i < countries.length; i++) {
		for (var j = 0; j < sortLeafNodes.length; j++) {
			if (sortLeafNodes[j].id != -1) {
				updateAreaTableData(countries[i].id, sortLeafNodes[j].id);
			}
		}
	}
}
...
function getAreaTableDataByCountryIdAndTableHeadNodeId(countryId,
		tableHeadNodeId) {
	$.ajax({
		type : "GET",
...
// 获取叶子结点排序
function getSortLeafNodes() {
	sortLeafNodes.splice(0, sortLeafNodes.length); // 清空数组
	console.log(sortLeafNodes); // 输出，空数组，即被清空了

	for (var i = 0; i < headNodes.length; i++) {
		if (headNodes[i].isLeaf == 1) {
			sortLeafNodes.push(headNodes[i]);
		}
	}

	for (var i = 0; i < sortLeafNodes.length; i++) {
		for (var j = 0; j < (sortLeafNodes[i].level - 1); j++) {
			sortLeafNodes[i].myGroup = sortLeafNodes[i].myGroup / 100;
		}
	}

	for (var i = 0; i < sortLeafNodes.length; i++) {
		for (var j = sortLeafNodes.length - 1; j > i; j--) {
			if (sortLeafNodes[j].myGroup < sortLeafNodes[j - 1].myGroup) {
				var temp = sortLeafNodes[j];
				sortLeafNodes[j] = sortLeafNodes[j - 1];
				sortLeafNodes[j - 1] = temp;
			}
		}
	}
}
...
function generatingTable() {
	...
	for (var i = 0; i < deepestNodeLevel; i++) {
		...
		for (var j = 0; j < levelNodes.length; j++) {
			myHtml += "<td id='" + levelNodes[j].id + "' rowspan='"
					+ levelNodes[j].height + "' colspan='"
					+ levelNodes[j].weight + "'>" + levelNodes[j].headNodeName
					+ "</td>";
		}
		myHtml += "</tr>";
	}
